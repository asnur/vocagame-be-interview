...
func (c *Controller) Deposit(fCtx *fiber.Ctx) error {
	...
	var req ibModel.DepositRequest
	if err := utils.ParseAndValidate(fCtx, &req); err != nil {
		return utils.Response(fCtx, fiber.StatusBadRequest, "Invalid request", nil, err)
	}

	deposit, err := c.UseCase.Wallet.Deposit(fCtx.UserContext(), req.ToUcModel(userID))
	if err != nil {
		status, err := pkgErr.ErrorResPonse(err)
		return utils.Response(fCtx, status, "Failed to deposit wallet", nil, err)
	}

	return utils.Response(fCtx, fiber.StatusCreated, "Wallet deposit successfully", deposit, nil)
}
